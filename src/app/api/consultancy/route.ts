...
function generateConsultancyEmailHtml(data: ConsultancyFormData): string {
  const styles = {
  ...
  const htmlContent = `
    <html>
      <head>
        ...
        <title>New Consultancy Request from ${data.name}</title>
      </head>
      <body style="${styles.body}">
        <div style="${styles.container}">
          <div style="${styles.header}">
            <h1 style="${styles.headerTitle}">CodeCafe Lab - New Consultancy Request</h1>
          </div>
          ...
          <div style="${styles.section}">
            ...
            <table style="${styles.table}">
              ...
              ${data.company ? `<tr><th style="${styles.th}">Company:</th><td style="${styles.td}">${data.company}</td></tr>` : ''}
            </table>
          </div>

          <div style="${styles.section}">
            <h2 style="${styles.sectionTitle}">Consultancy Inquiry</h2>
          ...
          <div style="${styles.footer}">
            <p>This email was generated from a consultancy request on the CodeCafe Lab website.</p>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validationResult = consultancyFormSchema.safeParse(rawData);
    ...
    const emailHtml = generateConsultancyEmailHtml(formData);
    const emailSubject = `New Consultancy Request from ${formData.name}`;
    ...
    try {
      ...
      const info = await transporter.sendMail({
        from: `"CodeCafe Lab Consultancy" <hello@codecafelab.in>`,
        to: recipientEmail,
        replyTo: formData.email,
        subject: emailSubject,
        html: emailHtml,
      });
      console.log("Consultancy form email sent: %s", info.messageId);
      return NextResponse.json({ 
        message: "Consultancy request submitted and email sent successfully!" 
      }, { status: 200 });
    } catch (emailError: any) {
      console.error('Error sending consultancy form email:', emailError);
    ...
    return NextResponse.json({ 
      message: 'Failed to process consultancy form.', 
      error: error.message 
    }, { status: 500 });
  }
} 