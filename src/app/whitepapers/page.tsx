...
export default function WhitepapersPage() {
  ...
  if (loading) return (
    <div className="container mx-auto py-12">
        <h1 className="text-3xl font-bold mb-8">Whitepapers Library</h1>
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => <WhitepaperSkeleton key={index} />)}
        </div>
    </div>
  );

  if (!items.length) return <div className="text-center py-12">No whitepapers found.</div>;
