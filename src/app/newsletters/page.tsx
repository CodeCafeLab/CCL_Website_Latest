...
import { User, Tag, Star, Calendar, Mail } from "lucide-react";
  ...
  if (loading) return (
    <div className="container mx-auto py-12">
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => <NewsletterSkeleton key={index} />)}
        </div>
    </div>
  );

  if (!items.length) return <div className="text-center py-12">No newsletters found.</div>;
      ...
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <div key={item.id} className="p-6 bg-card rounded-lg shadow flex flex-col">
            ...
            <h2 className="text-xl font-semibold mb-2">{item.title || "Newsletter"}</h2>
            <div className="mb-2 text-sm text-muted-foreground flex flex-wrap gap-2">
              ...
              {item.scheduled_at && (
                <span className="flex items-center gap-1">
                  <Calendar className="h-4 w-4" /> Scheduled: {new Date(item.scheduled_at).toLocaleDateString()}
                </span>
              )}
              {item.sent_at && (
                <span className="flex items-center gap-1">
                  <Mail className="h-4 w-4" /> Sent: {new Date(item.sent_at).toLocaleDateString()}
                </span>
              )}
            </div>
            <p className="mb-2 line-clamp-3">{item.subject_line}</p>
