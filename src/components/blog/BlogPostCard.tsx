    ...
    <Card className="group flex flex-col h-full overflow-hidden transition-all duration-300 ease-in-out hover:shadow-2xl hover:-translate-y-1">
      <Link href={`/blog/${post.id}`} className="block">
        <CardHeader className="p-0">
          <div className="relative aspect-video overflow-hidden">
            <Image
              src={post.thumbnail || 'https://placehold.co/600x400.png'}
              alt={post.title}
              fill
      ...
      <CardContent className="p-6 flex-grow">
        ...
        <div className="flex items-center text-xs text-muted-foreground mb-3 gap-2">
          ...
          <span>{new Date(post.createdDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
        </div>
        <CardDescription className="text-sm line-clamp-3">{post.summary}</CardDescription>
      </CardContent>
