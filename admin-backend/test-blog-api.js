...
const testBlogData = {
  ...
  "content": "React Hooks are a powerful feature that allows you to use state and other React features in functional components. In this comprehensive guide, we'll explore useState, useEffect, and other essential hooks.",
  "author": "John Doe",
  "author_id": "1",
  "status": "published",
  "category": "React",
...
async function testGetAllBlogs() {
  try {
    ...
    console.error('Error getting blogs:', error.response?.data || error.message);
  }
}
...
async function testCreateBlog() {
  try {
    ...
    console.log('Note: This requires authentication. You may need to login first.');
    ...
    const response = await axios.post('http://localhost:5000/api/blogs', testBlogData, {
      headers: {
...
async function testLogin() {
  try {
    ...
    console.log('You may need to register a user first or check your credentials');
  }
}
...
async function runTests() {
  ...
  if (token) {
    ...
    try {
      ...
      console.error('Error creating blog with token:', error.response?.data || error.message);
    }
  }
